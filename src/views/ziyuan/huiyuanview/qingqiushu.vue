		...
		<el-card shadow="hover">
			...
			<div style="display:flex;">
				<div style="text-align:left; width:50%;">
					<span v-if="radio2 == '回源请求数'">回源总请求数：0次</span>
					<span v-if="radio2 == '回源QPS'">回源峰值QPS：0次/秒</span>
				</div>
				<div style="text-align:right; width:50%">
					<el-radio-group v-model="radio2">
						<el-radio-button label="回源请求数" size="default" />
						<el-radio-button label="回源QPS" size="default" />
					</el-radio-group>
				</div>
	...
	export default defineComponent({
		setup() {
			...
			watch([radio2], (newValue: any, oldValue: any) => { //直接监听
				if (radio2.value == '回源请求数') {
					reads1();
				} else if (radio2.value == '回源QPS') {
					reads2();
				}
			});
